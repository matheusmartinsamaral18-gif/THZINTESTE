...
interface KidsSeriesTabProps {
  onItemClick: (item: any) => void;
}

const KidsSeriesTab: React.FC<KidsSeriesTabProps> = ({ onItemClick }) => {
      ...
      try {
        ...
        const fetchBatch = async (start: number) => {
          const promises = [0, 1, 2, 3].map(offset => 
            fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&language=pt-BR&sort_by=popularity.desc&page=${start + offset}&with_genres=16`)
              .then(res => res.json())
          );
        ...
        console.error("Erro ao carregar séries kids:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="px-4 md:px-12 py-6">
        <div className="flex items-center gap-4 mb-10">
          ...
          <h1 className="text-2xl md:text-4xl font-black uppercase italic tracking-tighter text-white">
            SÉRIES <span className="text-sky-500">ANIMADAS</span>
          </h1>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-sky-500 animate-pulse italic">Sincronizando Diversão...</p>
          </div>
        ) : (
