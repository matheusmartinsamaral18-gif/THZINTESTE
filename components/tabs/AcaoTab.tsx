...
interface AcaoTabProps {
  onItemClick: (item: any) => void;
}

const AcaoTab: React.FC<AcaoTabProps> = ({ onItemClick }) => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const data = await tmdbService.getMoviesByGenre(28, 1);
        setMovies(data || []);
      } catch (error) {
        console.error(error);
      } finally {
    ...
    <div className="px-4 md:px-12 py-6 animate-in fade-in duration-700">
      <div className="flex items-center gap-4 mb-12">
        ...
        <h1 className="text-2xl md:text-4xl font-black uppercase italic tracking-tighter text-white">
          CATÁLOGO <span className="text-rose-600">AÇÃO</span>
        </h1>
      </div>
